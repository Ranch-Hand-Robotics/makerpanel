...
// Opening edges are this far inward from the mounting-hole center lines.
openingInset = 5; // [3.6:0.1:20]
...
opening_size = trackball_hole_pitch - [2 * openingInset, 2 * openingInset];
edge_clearance = 2;
...
module validate_trackball_panel() {
	...
	assert(openingInset >= mount_diameter / 2 + edge_clearance,
		"Opening must stay at least 2 mm inside the mounting-hole edges.");
